...
int main()
{
	...
	while (window.isOpen())
	{
		...
		//std::cout << std::format("time error (micros s) : {0:.5f}\n", ((1.0 / 62.0) - timeSinceLastFrame.asSeconds()) * 1000.0);
		...
		text.setString(std::format("FPS : {0:.5f}", 1.0 / timeSinceLastFrame.asSeconds()));
